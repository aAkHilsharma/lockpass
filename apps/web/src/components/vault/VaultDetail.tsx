...
function FieldRow({ icon, label, value, mono = false, isLink = false, actions }: {
  icon: React.ReactNode; label: string; value: string;
  mono?: boolean; isLink?: boolean; actions?: React.ReactNode;
}) {
  return (
      ...
      <div className="flex-1 min-w-0">
        ...
        {isLink ? (
          <a href={value} target="_blank" rel="noopener noreferrer"
            className="text-[14px] text-accent hover:underline break-all">{value}</a>
        ) : (
...
export function VaultDetail({ item, isTrash = false, onEdit, onMoveToTrash, onRestore, onDeletePermanently, onNew }: Props) {
  const [showPw, setShowPw] = useState(false);
  const [showMenu, setShowMenu] = useState(false);
  const [modal, setModal] = useState<'trash' | 'permanent' | null>(null);
  const [flash, flashMsg] = useFlash();
  const menuRef = useRef<HTMLDivElement>(null);

  useEffect(() => { setShowPw(false); setShowMenu(false); }, [item?.id]);
    ...
    <section className="flex-1 flex flex-col min-w-0 overflow-hidden bg-bg">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-rule-soft shrink-0">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {isTrash ? (
            /* Trash mode — Restore + Delete permanently */
            <>
              <button
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-red-200 text-red-600 text-[13px] font-medium hover:bg-red-50 transition-colors"
              >
                <Icons.Trash s={13} />
                Delete permanently
              </button>
            ...
            /* Normal mode — Edit + three-dot menu */
            <>
              <button
              ...
              <div ref={menuRef} className="relative">
                ...
                {showMenu && (
                  <div className="absolute top-full right-0 mt-1 w-40 bg-bg-elev border border-rule rounded-lg shadow-lg overflow-hidden z-50">
                    <button
                      onClick={() => { setShowMenu(false); onMoveToTrash(item); }}
                      className="w-full px-3 py-2.5 text-[13px] text-left text-ink-2 hover:bg-bg-sunk transition-colors flex items-center gap-2"
                    >
                      <Icons.Trash s={13} />
                      Move to trash
                    </button>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <>
            <div className="mx-5 mt-5 rounded-xl border border-rule-soft bg-bg-elev overflow-hidden">
              <div className="flex items-start gap-3.5 px-5 py-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[11.5px] text-ink-3 mb-0.5">Email or username</p>
                  {item.username ? (
                    <p className="text-[14px] text-ink break-all">{item.username}</p>
                  ) : (
                    <p className="text-[14px] text-ink-4">None</p>
                  )}
                </div>
                {item.username && (
                  <div className="shrink-0 flex items-center gap-1">
                    <button onClick={() => copy(item.username!, 'Username')}
              ...
              <div className="flex items-start gap-3.5 px-5 py-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.password ? (
                    <p className="text-[14px] text-ink font-mono">
                      {showPw ? item.password : maskedDots(item.password.length)}
                    </p>
                  ) : (
                    <p className="text-[14px] text-ink-4">None</p>
                  )}
                ...
                {item.password && (
                  <div className="shrink-0 flex items-center gap-1">
                    ...
                    <button onClick={() => copy(item.password!, 'Password')}
                      className="w-7 h-7 flex items-center justify-center rounded-md text-ink-4 hover:bg-bg-sunk hover:text-ink transition-colors">
                      <Icons.Copy s={13} />
                    </button>
                  </div>
                    ...
                    <div className="flex flex-col gap-1">
                      {item.urls.map((u, i) => (
                        <a key={i} href={u} target="_blank" rel="noopener noreferrer"
                          className="text-[14px] text-accent hover:underline break-all">{u}</a>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        {item.notes && (
          <div className="mx-5 mt-5 rounded-xl border border-rule-soft bg-bg-elev overflow-hidden">
            <FieldRow icon={<Icons.Note s={16} />} label="Note" value={item.notes} />
          </div>
        )}
      ...
      {modal === 'permanent' && (
        <ConfirmModal
          ...
          cancelLabel="Cancel"
          danger
          onConfirm={() => { setModal(null); onDeletePermanently(item.id); }}
